...
class PsuUtil(PsuBase):
    ...
    def __init__(self):
        ...
        self.psu_eeprom_presence = self.psu_path + "eeprom/psu{}_vpd"
    ...
    def get_psu_dummy_status(self, index):
        """
        Retrieves dummy status of power supply unit (PSU) defined
                by 1-based index <index>

        :param index: An integer, 1-based index of the PSU of which to query status
        :return: Boolean, True if PSU is dummy, False if not
        """
        if index is None:
            raise RuntimeError("index shouldn't be None")
        if index > self.MAX_NUM_PSU:
        ...
        # A dummy PSU is present, but does not have power or eeprom
        if not self.get_psu_presence(index):
            return False
        psu_eeprom_presence = os.path.isfile(self.psu_eeprom_presence.format(index))
        if (not self.get_psu_status(index)) and (not psu_eeprom_presence):
            return True
        return False
